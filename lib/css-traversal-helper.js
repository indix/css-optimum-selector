...
var CssTraversalHelper = function (_CssCheckListHelper) {
  ...
  _createClass(CssTraversalHelper, [{
    ...
    value: function formPriorityArray(checkList) {
      ...
      var priorityArray = [];
      Object.keys(checkList).forEach(function (cur) {
        return priorityArray.push.apply(priorityArray, _toConsumableArray(_this2.composePathStringHelper(cur, checkList[cur])));
      });
      ...
      for (var i = 0; i < priorityArray.length; i++) {
        for (var j = i; j < priorityArray.length; j++) {
          ...
          if (result) {
            ...
            if (isUniqueInDocument(parent, result)) return result;
          }
        }
